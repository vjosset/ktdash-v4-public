...
import { isBattleOutcome } from '@/types'
import { NextResponse } from 'next/server'

// Report a battle. The reporter always goes into slot A.
export async function POST(req: Request) {
  ...
  if (!isBattleOutcome(result)) {
    return NextResponse.json({ error: 'Invalid battle outcome.' }, { status: 400 })
  }
  ...
  // Rule 1: only a roster owner may report a battle involving their roster
  ...
  const outcome = await BattleService.createBattle({
    rosterA,
    rosterB,
    result,
  ...
  return NextResponse.json(outcome.battle.toPlain())
}
